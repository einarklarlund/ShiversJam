...
Shader "psx/VertexLit-Retro3D transparent layered" {
	Properties{
		[Header(Retro3D parameters)]
		_MainTex("Main texture (_MainTex)", 2D) = "white" {}
		_OverlayTex("Overlay texture (_OverlayTex)", 2D) = "white" {}
		...
		SubShader {
			...
			v2f vert(appdata_full v)
			{
                ...
                o.uv_OverlayTex = TRANSFORM_TEX(v.texcoord, _OverlayTex);
                o.uv_OverlayTex *= 1 + _AffineStrength * (distance + (wp.w*(UNITY_LIGHTMODEL_AMBIENT.a * 16)) / distance / 2);
			...
			float4 frag(v2f i) : COLOR
			{
				...
				// get colors from main texture and overlay texture
                float4 mainTexSample = tex2D(_MainTex, i.uv_MainTex / i.normal.r);
                float4 overlayTexSample = tex2D(_OverlayTex, i.uv_OverlayTex / i.normal.r);
				// float4 col = tex2D(_MainTex, i.uv_MainTex / i.normal.r) * i.color * _Color;
                // blend colors from main texture and overlay texture by the alpha value of the overlay
				float4 col = lerp(mainTexSample, overlayTexSample, overlayTexSample.a) * i.color * _Color;
